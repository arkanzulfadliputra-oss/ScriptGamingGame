...
local Toggle = Tab:CreateToggle({
   ...
   Callback = function(Value)
          if Value then
            ...
            trackerFolder.Name = "Tracker_3D"
            trackerFolder.Parent = workspace
            
            local function createTracker(monster)
                local box = Instance.new("BoxHandleAdornment")
                box.Name = "Box_" .. monster.Name
                box.Adornee = monster
                box.Size = monster:GetExtentsSize()
                box.Color3 = Color3.new(1, 0, 0) -- Merah
                box.Transparency = 0.5
                box.ZIndex = 10
                box.AlwaysOnTop = true
                box.Parent = trackerFolder
                
                local billboard = Instance.new("BillboardGui")
                billboard.Name = "Name_" .. monster.Name
                billboard.Adornee = monster
                billboard.Size = UDim2.new(0, 100, 0, 50)
                billboard.StudsOffset = Vector3.new(0, 3, 0)
                billboard.AlwaysOnTop = true
                billboard.Parent = trackerFolder
            ...
            line.Parent = trackerFolder
            
            spawn(function()
                while Value do
                    -- Update line ke monster terdekat
                    ...
                    for _, v in pairs(workspace:GetDescendants()) do
                        if v:IsA("Model") and v:FindFirstChild("Humanoid") then
                            ...
                            if name:find("rush") or name:find("ambush") or name:find("eyes") then
                                if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                                    local dist = (v:GetPivot().Position - player.Character.HumanoidRootPart.Position).Magnitude
                                ...
                                if not trackerFolder:FindFirstChild("Box_" .. v.Name) then
                                    createTracker(v)
                                end
                            end
                        end
                    ...
                    if closestMonster and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                        local startPos = player.Character.HumanoidRootPart.Position
                        local endPos = closestMonster:GetPivot().Position
                        local midPos = (startPos + endPos) / 2
                        local distance = (startPos - endPos).Magnitude
                        
                        line.Size = Vector3.new(0.2, 0.2, distance)
                        line.CFrame = CFrame.lookAt(midPos, endPos) * CFrame.new(0, 0, -distance/2)
                        line.Visible = true
                    else
                        line.Visible = false
                    end
            ...
            local trackerFolder = workspace:FindFirstChild("Tracker_3D")
            if trackerFolder then
                trackerFolder:Destroy()
            end

            print("Tracker 3D: OFF")
   end,
})
